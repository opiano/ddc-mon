...
onMounted(() => subscribeToType('AO'))
onUnmounted(() => unsubscribeFromType('AO'))
  ...
  <div class="flex flex-col h-full bg-[#060e20]">
    ...
    <div class="px-6 py-2 flex items-center justify-between border-b border-outline-variant/10 bg-surface-container-low/30">
      <div class="flex items-center gap-4">
        <h1 class="headline-font text-2xl font-bold text-on-surface tracking-tight">Analog Outputs</h1>
        ...
        <tbody class="divide-y divide-outline-variant/5">
          <tr v-if="objects.length === 0">
            <td colspan="9" class="text-center py-8 text-slate-500">
              ...
              <div v-else>No Analog Output objects found.</div>
            </td>
          </tr>
      ...
      <div class="bg-surface-container-high rounded-lg shadow-2xl border border-outline-variant/20 w-full max-w-md overflow-hidden">
        ...
        <div class="p-6 space-y-4">
          ...
          <div>
            ...
            <input :disabled="isNullChecked" type="number" step="any" v-model="controlValue" class="w-full bg-surface-container-highest border border-outline-variant/20 rounded px-3 py-2 text-on-surface focus:outline-none focus:border-primary disabled:opacity-40 disabled:cursor-not-allowed transition-colors" />
          </div>
          <div>
