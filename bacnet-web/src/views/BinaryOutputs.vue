...
onMounted(() => subscribeToType('BO'))
onUnmounted(() => unsubscribeFromType('BO'))
...
const submitAlarmConfig = () => {
  ...
  const payload = `${alarmEventDetection.value ? 1 : 0}`
  ...
  <div class="flex flex-col h-full bg-[#060e20]">
    ...
    <div class="px-6 py-2 flex items-center justify-between border-b border-outline-variant/10 bg-surface-container-low/30">
      <div class="flex items-center gap-4">
        <h1 class="headline-font text-2xl font-bold text-on-surface tracking-tight">Binary Outputs</h1>
        ...
        <tbody class="divide-y divide-outline-variant/5">
          <tr v-if="objects.length === 0">
            <td colspan="9" class="text-center py-8 text-slate-500">
              ...
              <div v-else>No Binary Output objects found.</div>
            </td>
          </tr>
