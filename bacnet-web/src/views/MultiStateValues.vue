  ...
  <div class="flex flex-col h-full bg-[#060e20]">
    ...
    <div class="px-6 py-2 flex items-center justify-between border-b border-outline-variant/10 bg-surface-container-low/30">
      <div class="flex items-center gap-4">
        <h1 class="headline-font text-2xl font-bold text-on-surface tracking-tight">Multi-state Values</h1>
      ...
      <table class="w-full text-left border-collapse high-density-table">
        <thead class="sticky top-0 z-20 bg-surface-container-high shadow-md">
          <tr>
            ...
            <th class="text-sm font-bold text-slate-500 uppercase tracking-widest pl-4 w-20">States</th>
        ...
        <tbody class="divide-y divide-outline-variant/5">
          ...
          <tr v-for="obj in paginatedObjects" :key="obj.id" @click="openControlModal(obj)" class="group hover:bg-surface-bright/50 transition-colors cursor-pointer" :class="{ 'bg-error/5': obj.sts === 'FAULT' || obj.sts === 'Fault' }">
            ...
            <td class="text-sm text-slate-400 font-mono py-1 pl-4 w-20">{{ obj.states }}</td>
    ...
    <div v-if="showAlarmModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div class="bg-surface-container-high rounded-lg shadow-2xl border border-outline-variant/20 w-full max-w-md overflow-hidden">
        ...
        <div class="p-6 space-y-4">
          ...
          <!-- Conditional Inputs (Alarm Value state index for MSV) -->
          <div class="space-y-4 pt-2">
            <div>
              <label class="block text-sm font-bold text-slate-400 mb-1">Alarm Value (State Index)</label>
              <input type="number" min="1" step="1" v-model="alarmValue" class="w-full bg-surface-container-highest border border-outline-variant/20 rounded px-3 py-2 text-on-surface focus:outline-none focus:border-primary transition-colors" />
            </div>
          </div>
        </div>
